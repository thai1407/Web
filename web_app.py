...
def print_msg(client, userdata, message):
    ...
    if 'dirt' in message.payload.decode('utf-8').strip():
        ...
        if float(dirt) != 0:
            with open('temp_dirt.txt', 'w') as f:
                f.write(dirt)
        with open('temp_air.txt', 'r') as f:
            air, temp = f.read().split(' ')
    if 'air' in message.payload.decode('utf-8').strip():
        air, temp = message.payload.decode('utf-8').strip().replace('air', "").split('-')
        if float(air) != 0:
            with open('temp_air.txt', 'w') as f:
                f.write(air + " " + temp)
        with open('temp_dirt.txt', 'r') as f:
            dirt = f.read()
        # dirt = message.payload.decode('utf-8').strip().replace('dirt', "")
